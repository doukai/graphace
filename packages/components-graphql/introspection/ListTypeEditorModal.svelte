<script lang="ts">
  ...
  import {
    querySubType,
    mutationSubType,
  } from "@packages/graphql/request/Type";
  ...
  const createItem = (): void => {
    ...
    value[__field.name] = [
      ...value[__field.name],
      manager.createTypeObject(__type),
    ];
  };
      ...
      select: (event) => {
        ...
        value[__field.name] = [
          ...
          ...event.detail.selectedDataList.filter(
            (selectedData) =>
              !value[__field.name].some(
                (data) => data[idFieldName] === selectedData[idFieldName]
              )
          ),
        ];
  ...
  <ModalContent>
    <div class="py-4 px-0 sm:px-6 lg:px-8 space-y-4">
      {#await queryPromise}
        {#each { length: 6 } as _}
          <div
            class="animate-pulse bg-base-200 px-12 py-12 sm:mt-px sm:pt-2 rounded"
          />
          <div class="divider" />
        {/each}
      {:then response}
        {#each value[__field.name] || [] as data, index}
          <div
            tabindex="0"
            class="collapse collapse-arrow  bg-base-100 rounded-box {errors[
              index
            ]
