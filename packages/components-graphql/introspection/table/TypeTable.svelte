<script lang="ts">
  ...
  const onNext = (
    event: CustomEvent<{ selectedPageSize: number; after: string }>
  ): void => {
  ...
  const onPrevious = (
    event: CustomEvent<{ selectedPageSize: number; before: string }>
  ): void => {
    pageSize = event.detail.selectedPageSize;
  ...
  const onSizeChange = (
    event: CustomEvent<{ selectedPageSize: number }>
  ): void => {
    pageSize = event.detail.selectedPageSize;
    refresh();
  };
