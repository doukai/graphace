<script lang="ts">
  ...
  import { type __Field, type __FieldFilter, type __Type, Operator } from '@graphace/graphql/types';
        ...
        {:else}
          ...
          {:else if manager.fieldIsEnum(__field.type)}
            {#if value.val[__field.name].opr === 'IN' || value.val[__field.name].opr === 'NIN' || value.val[__field.name].opr === 'BT' || value.val[__field.name].opr === 'NBT'}
              <CheckboxGroup
                bind:value="{value.val[__field.name].val}"
                checkboxs="{manager.getFieldTypeEnumValues(__field.type).map((enumValue) => {
                  return { name: enumValue.name, value: enumValue.name };
                })}"
              />
            {:else}
              <Select name="{__field.name}" bind:value="{value.val[__field.name].val}">
