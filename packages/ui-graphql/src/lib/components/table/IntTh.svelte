<script lang="ts">
	...
	import IntFilter from '../input/IntFilter.svelte';
	...
	let _value: NumberExpression = { opr: 'EQ', val: undefined, arr: [] };
	let _sort: Sort | undefined = undefined;

	const dispatch = createEventDispatcher<{
		filter: { value: NumberExpression | null | undefined; sort: Sort | null | undefined };
	}>();
...
{#if $open}
	...
	<div class="rounded-box shadow bg-base-200 p-1 z-[{zIndex + 5}]" use:melt={$content}>
		...
		>
			<IntFilter class="w-full" {disabled} bind:value={_value} />
