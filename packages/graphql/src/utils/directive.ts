...
export const buildDirectives = (directives?: Directive[] | undefined) => directives?.map(buildDirective).join(' ') ?? '';

export const buildDirective = (directive: Directive) => `@${directive.name}${buildArgs(directive.args)}`;
