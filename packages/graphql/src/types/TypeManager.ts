...
export class TypeManager {
    ...
    public getFieldTypeName(__type: __Type): string | undefined {
        if (__type.kind === __TypeKind.NON_NULL) {
    ...
    public getFieldTypeEnumValues(__type: __Type): __EnumValue[] | undefined {
        if (__type.kind === __TypeKind.NON_NULL) {
    ...
    public getNameFiled(__type: __Type): __Field | undefined {
        return _.find(this.getScalarFiledList(__type), field => field.name === "name") ||
            _.find(this.getScalarFiledList(__type), field => field.name === this.getIdFieldName(__type));
    }
    ...
    public getQueryTypeListFieldName(__type: __Type): string | undefined {
        if (__type.name) {
            if (_.startsWith(__type.name, "__")) {
    ...
    public getQueryTypeConnectionFieldName(__type: __Type): string | undefined {
        if (__type.name) {
            if (_.startsWith(__type.name, "__")) {
    ...
    public getQueryTypeFieldName(__type: __Type): string | undefined {
        if (__type.name) {
            if (_.startsWith(__type.name, "__")) {
    ...
    public getMutationTypeFieldName(__type: __Type): string | undefined {
        if (__type.name) {
            if (_.startsWith(__type.name, "__")) {
                return "__" + _.camelCase(__type.name.replace("__", ""));
            } else {
    ...
    public fieldTypeToArgumentType(__type: __Type): string | undefined {
        const fieldTypeName = this.getFieldTypeName(__type);
        if (__type.name && fieldTypeName) {
    ...
    public isContainerType(__type: __Type): boolean {
        return containerTypeSuffix.some(suffix => _.endsWith(this.getFieldTypeName(__type), suffix) || containerTypeList.some(containerType => this.getFieldTypeName(__type) === containerType));
    }
