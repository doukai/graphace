<script lang="ts">
	...
	import {
		...
		UserProfile,
		UserProfileOrderBy,
		QueryTypeUserProfileConnectionArgs,
	...
	let args: QueryTypeUserProfileConnectionArgs = {};
	let orderBy: UserProfileOrderBy = {};
	...
	const search = () => {
		...
		if (searchValue) {
			...
			args.qq = { opr: Operator.LK, val: `%${searchValue}%` };
	...
	const updateField = (args: MutationTypeUserProfileArgs | null | undefined) => {
		if (args) {
			dispatch('mutation', {
				args,
				then: (data) => {
				...
				{#each nodes as node, row}
					{#if node && node.id}
						<tr class="hover">
							...
							/>
							<ObjectTd name="user" errors={errors[row]?.iterms?.user} path={`${node.id}/user`} on:gotoField />
							<StringTd
								name="userId"
