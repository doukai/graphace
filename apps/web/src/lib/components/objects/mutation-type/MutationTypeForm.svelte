<script lang="ts">
	...
	import type { MutationType, MutationTypeMutationTypeArgs } from '~/gql/generated/schema';

	export let node: MutationTypeMutationTypeArgs | null | undefined;
	...
	const save = (): void => {
		if (node) {
			validate('MutationType', node, $locale)
				.then((data) => {
					errors = {};
					if (node) {
						dispatch('mutation', {
