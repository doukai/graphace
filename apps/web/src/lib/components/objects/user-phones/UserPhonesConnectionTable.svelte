<script lang="ts">
	...
	import {
		...
		UserPhones,
		UserPhonesOrderBy,
		QueryTypeUserPhonesConnectionArgs,
	...
	const dispatch = createEventDispatcher<{
		...
		edit: { id: string };
		create: {};
		save: { nodes: (UserPhones | null | undefined)[] | null | undefined };
		back: {};
	}>();

	let showDeleteButton = false;
	let searchValue: string | undefined;
	let args: QueryTypeUserPhonesConnectionArgs = {};
	...
	const updateField = (args: MutationTypeUserPhonesArgs | null | undefined) => {
		if (args) {
			dispatch('mutation', {
				args,
				then: (data) => {
					...
					{#if node && node.id}
						<tr class="hover">
							...
							<StringTd
								...
								errors={errors[row]?.iterms?.phone}
							/>
							<StringTd
								name="realmId"
