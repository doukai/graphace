<script lang="ts">
	...
	const dispatch = createEventDispatcher<{
		...
		parentMutation: {
			args: MutationTypeUserRoleArgs | null;
			update?: boolean;
			then: (data: UserRole | null | undefined) => void;
			catch: (errors: Errors) => void;
		};
	...
	>
		...
		{:else}
			{#if node}
				...
				<ObjectItem name="from" path={`${node.id}/from`} label={$LL.graphql.objects.UserRole.fields.from.name()} errors={errors.from} on:gotoField />
				<IntItem label={$LL.graphql.objects.UserRole.fields.roleId.name()} name="roleId" bind:value={node.roleId} errors={errors.roleId} />
				<ObjectItem name="to" path={`${node.id}/to`} label={$LL.graphql.objects.UserRole.fields.to.name()} errors={errors.to} on:gotoField />
