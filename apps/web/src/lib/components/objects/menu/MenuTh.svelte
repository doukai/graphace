<script lang="ts">
	...
	import MenuTypeInput from '~/lib/components/enums/menu-type/MenuTypeInput.svelte';
	import type { StringExpression } from '@graphace/graphql/types';
	import LL from '$i18n/i18n-svelte';
	import type { MenuTypeExpression, MenuExpression } from '$houdini';

	export let name: string;
	export let expression: MenuExpression | null | undefined;
	...
	const filter = (): void => {
		...
		if (_expression.icon.val || (_expression.icon.in && _expression.icon.in.length > 0)) {
			expression = { ...expression, icon: _expression.icon };
		} else {
			expression = { ...expression, icon: undefined };
		}
	...
	const iconOprChange = (): void => {
		_expression.icon.in = [];
		_expression.icon.val = undefined;
	};
		...
		<div class="grid grid-cols-2 gap-2">
			<div class="join">
				<button class="btn btn-active btn-ghost join-item w-1/3">
					{$LL.graphql.objects.Menu.fields.name.name()}
				</button>
				<select
			...
			<div class="join">
				<button class="btn btn-active btn-ghost join-item w-1/3">
					{$LL.graphql.objects.Menu.fields.type.name()}
				</button>
				<select
					class="select select-bordered join-item w-2/3"
			...
			<div class="join">
				<button class="btn btn-active btn-ghost join-item w-1/3">
					{$LL.graphql.objects.Menu.fields.path.name()}
				</button>
				<select
					class="select select-bordered join-item w-2/3"
			...
			<div class="join">
				<button class="btn btn-active btn-ghost join-item w-1/3">
					{$LL.graphql.objects.Menu.fields.icon.name()}
				</button>
				<select
					class="select select-bordered join-item w-2/3"
					bind:value={_expression.icon.opr}
					on:change={iconOprChange}
				>
					<option value="EQ" selected>{$LL.uiGraphql.table.th.eq()}</option>
					<option value="NEQ">{$LL.uiGraphql.table.th.neq()}</option>
					<option value="LK">{$LL.uiGraphql.table.th.lk()}</option>
			...
			{#if _expression.icon.opr === 'IN' || _expression.icon.opr === 'NIN' || _expression.icon.opr === 'BT' || _expression.icon.opr === 'NBT'}
				<StringInput
					placeholder={$LL.uiGraphql.table.th.filterPlaceholder()}
					{name}
					bind:value={_expression.icon.in}
				/>
			{:else}
				<StringInput
					placeholder={$LL.uiGraphql.table.th.filterPlaceholder()}
					{name}
					bind:value={_expression.icon.val}
				/>
			{/if}
			<div class="join">
				<button class="btn btn-active btn-ghost join-item w-1/3">
					{$LL.graphql.objects.Menu.fields.description.name()}
				</button>
				<select
					class="select select-bordered join-item w-2/3"
