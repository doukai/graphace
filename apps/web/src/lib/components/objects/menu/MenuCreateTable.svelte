<script lang="ts">
	...
	export let nodes: (MutationTypeMenuArgs | null | undefined)[] | null | undefined;
	...
	const dispatch = createEventDispatcher<{
		edit: { row: number };
		create: {};
		mutation: { nodes: (MutationTypeMenuArgs | null | undefined)[] | null | undefined };
		save: { nodes: (MutationTypeMenuArgs | null | undefined)[] | null | undefined };
		gotoSelect: {};
		back: {};
	}>();
	...
	<Table>
		<thead>
			<tr>
				...
				<td>{$LL.graphql.objects.Menu.fields.name.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.type.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.path.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.icon.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.folder.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.description.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.role.name()}</td>
				<td>{$LL.graphql.objects.Menu.fields.realm.name()}</td>
				<th />
			</tr>
		</thead>
		<tbody>
				...
				{#each nodes as node, row}
					{#if node}
						<tr class="hover">
							...
							/>
							<ObjectTd name="folder" errors={errors[row]?.iterms?.folder} path="_/folder" on:gotoField />
							<StringTd
								name="description"
