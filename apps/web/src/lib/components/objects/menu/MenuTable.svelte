<script lang="ts">
	...
	import type {
		Menu,
		MenuOrderBy,
		QueryTypeMenuListArgs,
		MutationTypeMenuArgs
	} from '~/lib/types/schema';

	export let nodes: (Menu | null | undefined)[] | null | undefined;
	...
	const dispatch = createEventDispatcher<{
		...
		edit: { id: string };
		create: {};
		save: { nodes: (Menu | null | undefined)[] | null | undefined };
		gotoSelect: {};
		back: {};
	}>();

	let args: QueryTypeMenuListArgs = {};
	...
	const search = (searchValue: string | undefined) => {
		...
		if (searchValue) {
			...
			args.icon = { opr: 'LK', val: `%${searchValue}%` };
	...
	const updateField = (args: MutationTypeMenuArgs | null | undefined) => {
		if (args) {
			dispatch('mutation', {
				args,
				update: true,
	...
	<Table>
		<thead>
			<tr>
				...
				<StringTh
					name={$LL.graphql.objects.Menu.fields.name.name()}
					bind:expression={args.name}
					bind:sort={orderBy.name}
					on:filter={query}
				/>
				<MenuTypeTh
					name={$LL.graphql.objects.Menu.fields.type.name()}
					bind:expression={args.type}
					bind:sort={orderBy.type}
					on:filter={query}
				/>
				<StringTh
					name={$LL.graphql.objects.Menu.fields.path.name()}
					bind:expression={args.path}
					bind:sort={orderBy.path}
					on:filter={query}
				/>
				<StringTh
					name={$LL.graphql.objects.Menu.fields.icon.name()}
					bind:expression={args.icon}
					bind:sort={orderBy.icon}
					on:filter={query}
				/>
				<MenuTh
					name={$LL.graphql.objects.Menu.fields.folder.name()}
					bind:expression={args.folder}
					on:filter={query}
				/>
				<StringTh
					name={$LL.graphql.objects.Menu.fields.description.name()}
					bind:expression={args.description}
					bind:sort={orderBy.description}
					on:filter={query}
				/>
				<RoleTh
					name={$LL.graphql.objects.Menu.fields.role.name()}
					bind:expression={args.role}
					on:filter={query}
				/>
				<RealmTh
					name={$LL.graphql.objects.Menu.fields.realm.name()}
						...
						{#if node && node.id}
							<tr class="hover">
								...
								<StringTd
									...
									on:save={() => updateField({ id: node?.id, icon: node?.icon })}
									errors={errors[row]?.iterms?.icon}
								/>
								<ObjectTd name="folder" errors={errors[row]?.iterms?.folder} path={`${node.id}/folder`} on:gotoField />
								<StringTd
									name="description"
