<script lang="ts">
	...
	import type {
		Api,
		ApiOrderBy,
		QueryTypeApiListArgs,
		MutationTypeApiArgs
	} from '~/lib/types/schema';

	export let nodes: (Api | null | undefined)[] | null | undefined;
	...
	const dispatch = createEventDispatcher<{
		...
		edit: { id: string };
		create: {};
		save: { nodes: (Api | null | undefined)[] | null | undefined };
		gotoSelect: {};
		back: {};
	}>();

	let args: QueryTypeApiListArgs = {};
	let orderBy: ApiOrderBy = {};
	...
	const updateField = (args: MutationTypeApiArgs | null | undefined) => {
		if (args) {
			dispatch('mutation', {
				args,
				update: true,
	...
	<Table>
		<thead>
			<tr>
				...
				<StringTh
					name={$LL.graphql.objects.Api.fields.name.name()}
					bind:expression={args.name}
					bind:sort={orderBy.name}
					on:filter={query}
				/>
				<StringTh
					name={$LL.graphql.objects.Api.fields.description.name()}
					bind:expression={args.description}
					bind:sort={orderBy.description}
					on:filter={query}
				/>
				<ApiTypeTh
					name={$LL.graphql.objects.Api.fields.type.name()}
					bind:expression={args.type}
					bind:sort={orderBy.type}
					on:filter={query}
				/>
				<RoleTh
					name={$LL.graphql.objects.Api.fields.role.name()}
					bind:expression={args.role}
					on:filter={query}
				/>
				<RealmTh
					name={$LL.graphql.objects.Api.fields.realm.name()}
