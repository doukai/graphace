...
<Card>
	...
	>
		...
		<ObjectItem name="from" path="_/from" label={$LL.graphql.objects.GroupRole.fields.from.name()} errors={errors.from} on:gotoField />
		<IntItem label={$LL.graphql.objects.GroupRole.fields.roleId.name()} name="roleId" bind:value={node.roleId} errors={errors.roleId} />
		<ObjectItem name="to" path="_/to" label={$LL.graphql.objects.GroupRole.fields.to.name()} errors={errors.to} on:gotoField />
