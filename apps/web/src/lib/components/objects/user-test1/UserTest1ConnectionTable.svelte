<script lang="ts">
	...
	import {
		...
		UserTest1,
		UserTest1OrderBy,
		QueryTypeUserTest1ConnectionArgs,
	...
	const dispatch = createEventDispatcher<{
		...
		edit: { id: string };
		create: {};
		save: { nodes: (UserTest1 | null | undefined)[] | null | undefined };
		back: {};
	}>();
	...
	let args: QueryTypeUserTest1ConnectionArgs = {};
	...
	async function updateField(args: MutationTypeUserTest1Args | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserTest1', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
