<script lang="ts">
	...
	const filter = (): void => {
		...
		if (_expression.ofTypeName.val || (_expression.ofTypeName.in && _expression.ofTypeName.in.length > 0)) {
			expression = { ...expression, ofTypeName: _expression.ofTypeName };
		} else {
			expression = { ...expression, ofTypeName: undefined };
		}
	...
	const ofTypeNameOprChange = (): void => {
		_expression.ofTypeName.in = [];
		_expression.ofTypeName.val = undefined;
	};
...
<div class="hidden">
	<div class="space-y-1" bind:this={content}>
		...
		<div class="flex items-start space-x-1">
			<select
				class="select select-bordered"
				bind:value={_expression.ofTypeName.opr}
				on:change={ofTypeNameOprChange}
			>
				<option value="EQ" selected>{$LL.uiGraphql.table.th.eq()}</option>
				<option value="NEQ">{$LL.uiGraphql.table.th.neq()}</option>
				<option value="LK">{$LL.uiGraphql.table.th.lk()}</option>
			...
			{#if _expression.ofTypeName.opr === 'IN' || _expression.ofTypeName.opr === 'NIN' || _expression.ofTypeName.opr === 'BT' || _expression.ofTypeName.opr === 'NBT'}
				<StringInput
					placeholder={$LL.uiGraphql.table.th.filterPlaceholder()}
					{name}
					bind:value={_expression.ofTypeName.in}
				/>
			{:else}
				<StringInput
					placeholder={$LL.uiGraphql.table.th.filterPlaceholder()}
					{name}
					bind:value={_expression.ofTypeName.val}
				/>
			{/if}
		</div>
		<div class="flex items-start space-x-1">
