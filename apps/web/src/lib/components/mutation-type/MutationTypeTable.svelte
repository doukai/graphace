<script lang="ts">
	...
	let orderBy: MutationTypeOrderBy = {};
	...
	async function updateField(args: MutationTypeMutationTypeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('MutationType', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="MutationType">
