<script lang="ts">
	...
	import {
		Conditional,
		Operator,
		UserPhonesConnection,
		UserPhonesConnectionOrderBy,
		QueryTypeUserPhonesConnectionListArgs,
		MutationTypeUserPhonesConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserPhonesConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserPhonesConnectionListArgs;
			then: (data: (UserPhonesConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserPhonesConnectionArgs;
			then: (data: UserPhonesConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserPhonesConnectionListArgs = {};
	let orderBy: UserPhonesConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserPhonesConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserPhonesConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserPhonesConnection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-phones-connection/${node.id}`);
											}
										}}
									>
