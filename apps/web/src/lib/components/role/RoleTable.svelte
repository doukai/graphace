<script lang="ts">
	...
	async function updateField(args: MutationTypeRoleArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('Role', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
					...
					{#if node && node.id}
						<tr class="hover">
							...
							<RoleTypeTd
								...
								on:save={() => updateField({ : node?., type: node?.type })}
								error={errors[node.]?.type}
							/>
							<TimestampTd
								name="updateTime"
