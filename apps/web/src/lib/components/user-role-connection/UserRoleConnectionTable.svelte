<script lang="ts">
	...
	import {
		...
		Operator,
		UserRoleConnection,
		UserRoleConnectionOrderBy,
		QueryTypeUserRoleConnectionListArgs,
		MutationTypeUserRoleConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserRoleConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserRoleConnectionListArgs;
			then: (data: (UserRoleConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserRoleConnectionArgs;
			then: (data: UserRoleConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserRoleConnectionListArgs = {};
	let orderBy: UserRoleConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserRoleConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserRoleConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserRoleConnection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-role-connection/${node.id}`);
											}
										}}
									>
