<script lang="ts">
	...
	import {
		...
		Operator,
		UserPhonesEdge,
		UserPhonesEdgeOrderBy,
		QueryTypeUserPhonesEdgeListArgs,
		MutationTypeUserPhonesEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserPhonesEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserPhonesEdgeListArgs;
			then: (data: (UserPhonesEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserPhonesEdgeArgs;
			then: (data: UserPhonesEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserPhonesEdgeListArgs = {};
	let orderBy: UserPhonesEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeUserPhonesEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserPhonesEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserPhonesEdge">
