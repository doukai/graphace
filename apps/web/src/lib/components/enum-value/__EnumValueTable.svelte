<script lang="ts">
	...
	import {
		...
		__EnumValue,
		__EnumValueOrderBy,
		QueryType__EnumValueListArgs,
		MutationType__EnumValueArgs
	} from '~/gql/generated/schema';

	export let nodes: (__EnumValue | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryType__EnumValueListArgs;
			then: (data: (__EnumValue | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationType__EnumValueArgs;
			then: (data: __EnumValue | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryType__EnumValueListArgs = {};
	let orderBy: __EnumValueOrderBy = {};
	...
	async function updateField(args: MutationType__EnumValueArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('__EnumValue', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
