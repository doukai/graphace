<script lang="ts">
	...
	import {
		...
		UserProfileConnectionOrderBy,
		QueryTypeUserProfileConnectionListArgs,
		MutationTypeUserProfileConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserProfileConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserProfileConnectionListArgs;
			then: (data: (UserProfileConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserProfileConnectionArgs;
			then: (data: UserProfileConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserProfileConnectionListArgs = {};
	let orderBy: UserProfileConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserProfileConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserProfileConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserProfileConnection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-profile-connection/${node.id}`);
											}
										}}
									>
