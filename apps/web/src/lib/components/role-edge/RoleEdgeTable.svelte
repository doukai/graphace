<script lang="ts">
	...
	import {
		...
		RoleEdge,
		RoleEdgeOrderBy,
		QueryTypeRoleEdgeListArgs,
		MutationTypeRoleEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (RoleEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeRoleEdgeListArgs;
			then: (data: (RoleEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeRoleEdgeArgs;
			then: (data: RoleEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeRoleEdgeListArgs = {};
	let orderBy: RoleEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeRoleEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('RoleEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
