<script lang="ts">
	...
	import {
		Conditional,
		Operator,
		RoleRoleTypeConnection,
		RoleRoleTypeConnectionOrderBy,
		QueryTypeRoleRoleTypeConnectionListArgs,
		MutationTypeRoleRoleTypeConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (RoleRoleTypeConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeRoleRoleTypeConnectionListArgs;
			then: (data: (RoleRoleTypeConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeRoleRoleTypeConnectionArgs;
			then: (data: RoleRoleTypeConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeRoleRoleTypeConnectionListArgs = {};
	let orderBy: RoleRoleTypeConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeRoleRoleTypeConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('RoleRoleTypeConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="RoleRoleTypeConnection">
	...
	<div class="tooltip tooltip-bottom" data-tip={$LL.routers.type.remove()}>
		<button
			...
			on:click={(e) => {
				e.preventDefault();
				goto('./role-role-type-connection/+');
			} }
		>
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./role-role-type-connection/${node.id}`);
											}
										}}
									>
