<script lang="ts">
	...
	import {
		...
		Operator,
		UserConnection,
		UserConnectionOrderBy,
		QueryTypeUserConnectionListArgs,
		MutationTypeUserConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserConnectionListArgs;
			then: (data: (UserConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserConnectionArgs;
			then: (data: UserConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserConnectionListArgs = {};
	let orderBy: UserConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserConnection">
