<script lang="ts">
	...
	import {
		...
		OrganizationConnectionOrderBy,
		QueryTypeOrganizationConnectionListArgs,
		MutationTypeOrganizationConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (OrganizationConnection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeOrganizationConnectionListArgs;
			then: (data: (OrganizationConnection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeOrganizationConnectionArgs;
			then: (data: OrganizationConnection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeOrganizationConnectionListArgs = {};
	let orderBy: OrganizationConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeOrganizationConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('OrganizationConnection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="OrganizationConnection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./organization-connection/${node.id}`);
											}
										}}
									>
