<script lang="ts">
	...
	import {
		...
		Operator,
		UserProfileEdge,
		UserProfileEdgeOrderBy,
		QueryTypeUserProfileEdgeListArgs,
		MutationTypeUserProfileEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserProfileEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserProfileEdgeListArgs;
			then: (data: (UserProfileEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserProfileEdgeArgs;
			then: (data: UserProfileEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserProfileEdgeListArgs = {};
	let orderBy: UserProfileEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeUserProfileEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserProfileEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserProfileEdge">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-profile-edge/${node.id}`);
											}
										}}
									>
