<script lang="ts">
	...
	import {
		...
		Operator,
		UserTest2Connection,
		UserTest2ConnectionOrderBy,
		QueryTypeUserTest2ConnectionListArgs,
		MutationTypeUserTest2ConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserTest2Connection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserTest2ConnectionListArgs;
			then: (data: (UserTest2Connection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserTest2ConnectionArgs;
			then: (data: UserTest2Connection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserTest2ConnectionListArgs = {};
	let orderBy: UserTest2ConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserTest2ConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserTest2Connection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserTest2Connection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-test2-connection/${node.id}`);
											}
										}}
									>
