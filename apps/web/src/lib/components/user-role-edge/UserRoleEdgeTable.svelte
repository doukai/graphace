<script lang="ts">
	...
	import {
		...
		UserRoleEdge,
		UserRoleEdgeOrderBy,
		QueryTypeUserRoleEdgeListArgs,
		MutationTypeUserRoleEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserRoleEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserRoleEdgeListArgs;
			then: (data: (UserRoleEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserRoleEdgeArgs;
			then: (data: UserRoleEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserRoleEdgeListArgs = {};
	let orderBy: UserRoleEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeUserRoleEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserRoleEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserRoleEdge">
