<script lang="ts">
	...
	import { IDTh, IDTd } from '@graphace/ui-graphql/components/table';
	...
	import {
		...
		Operator,
		RoleRoleTypeEdge,
		RoleRoleTypeEdgeOrderBy,
		QueryTypeRoleRoleTypeEdgeListArgs,
		MutationTypeRoleRoleTypeEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (RoleRoleTypeEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeRoleRoleTypeEdgeListArgs;
			then: (data: (RoleRoleTypeEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeRoleRoleTypeEdgeArgs;
			then: (data: RoleRoleTypeEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeRoleRoleTypeEdgeListArgs = {};
	let orderBy: RoleRoleTypeEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeRoleRoleTypeEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('RoleRoleTypeEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="RoleRoleTypeEdge">
...
<Table>
	<thead>
		<tr>
			...
			<IDTh
				name="cursor"
				bind:expression={args.cursor}
				bind:sort={orderBy.cursor}
				on:filter={query}
			/>
			<td />
		</tr>
				...
				{#each nodes as node}
					{#if node && node.id}
						<tr class="hover">
							...
							<IDTd
								name="cursor"
								bind:value={node.cursor}
								on:save={() => updateField({ : node?., cursor: node?.cursor })}
								error={errors[node.]?.cursor}
							/>
							<td>
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./role-role-type-edge/${node.id}`);
											}
										}}
									>
