<script lang="ts">
	...
	import {
		...
		Operator,
		OrganizationEdge,
		OrganizationEdgeOrderBy,
		QueryTypeOrganizationEdgeListArgs,
		MutationTypeOrganizationEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (OrganizationEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeOrganizationEdgeListArgs;
			then: (data: (OrganizationEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeOrganizationEdgeArgs;
			then: (data: OrganizationEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeOrganizationEdgeListArgs = {};
	let orderBy: OrganizationEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeOrganizationEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('OrganizationEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="OrganizationEdge">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./organization-edge/${node.id}`);
											}
										}}
									>
