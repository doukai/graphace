<script lang="ts">
	...
	async function updateField(args: MutationTypeUserRoleArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserRole', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
					...
					{#if node && node.id}
						<tr class="hover">
							...
							<IntTd
								...
								on:save={() => updateField({ : node?., roleId: node?.roleId })}
								error={errors[node.]?.roleId}
							/>
							<TimestampTd
								name="updateTime"
