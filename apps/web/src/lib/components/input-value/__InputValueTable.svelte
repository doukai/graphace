<script lang="ts">
	...
	import {
		...
		__InputValue,
		__InputValueOrderBy,
		QueryType__InputValueListArgs,
		MutationType__InputValueArgs
	} from '~/gql/generated/schema';

	export let nodes: (__InputValue | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryType__InputValueListArgs;
			then: (data: (__InputValue | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationType__InputValueArgs;
			then: (data: __InputValue | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryType__InputValueListArgs = {};
	let orderBy: __InputValueOrderBy = {};
	...
	const query = () => {
		if (queryValue) {
			...
			args.defaultValue = { opr: Operator.LK, val: `%${queryValue}%` };
	...
	async function updateField(args: MutationType__InputValueArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('__InputValue', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="__InputValue">
...
<Table>
	<thead>
		<tr>
			...
			<StringTh
				name="defaultValue"
				bind:expression={args.defaultValue}
				bind:sort={orderBy.defaultValue}
				on:filter={query}
			/>
			<BooleanTh
				name="isDeprecated"
				...
				{#each nodes as node}
					{#if node && node.id}
						<tr class="hover">
							...
							<StringTd
								name="defaultValue"
								bind:value={node.defaultValue}
								on:save={() => updateField({ : node?., defaultValue: node?.defaultValue })}
								error={errors[node.]?.defaultValue}
							/>
							<BooleanTd
								name="isDeprecated"
