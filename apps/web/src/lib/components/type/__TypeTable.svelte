<script lang="ts">
	...
	import {
		...
		__Type,
		__TypeOrderBy,
		QueryType__TypeListArgs,
		MutationType__TypeArgs
	} from '~/gql/generated/schema';

	export let nodes: (__Type | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryType__TypeListArgs;
			then: (data: (__Type | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationType__TypeArgs;
			then: (data: __Type | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryType__TypeListArgs = {};
	let orderBy: __TypeOrderBy = {};
	...
	async function updateField(args: MutationType__TypeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('__Type', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
