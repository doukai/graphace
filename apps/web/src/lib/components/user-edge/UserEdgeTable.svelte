<script lang="ts">
	...
	import {
		...
		UserEdge,
		UserEdgeOrderBy,
		QueryTypeUserEdgeListArgs,
		MutationTypeUserEdgeArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserEdge | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserEdgeListArgs;
			then: (data: (UserEdge | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserEdgeArgs;
			then: (data: UserEdge | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserEdgeListArgs = {};
	let orderBy: UserEdgeOrderBy = {};
	...
	async function updateField(args: MutationTypeUserEdgeArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserEdge', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
