<script lang="ts">
	...
	import {
		...
		Operator,
		UserTest1Connection,
		UserTest1ConnectionOrderBy,
		QueryTypeUserTest1ConnectionListArgs,
		MutationTypeUserTest1ConnectionArgs
	} from '~/gql/generated/schema';

	export let nodes: (UserTest1Connection | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryTypeUserTest1ConnectionListArgs;
			then: (data: (UserTest1Connection | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationTypeUserTest1ConnectionArgs;
			then: (data: UserTest1Connection | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryTypeUserTest1ConnectionListArgs = {};
	let orderBy: UserTest1ConnectionOrderBy = {};
	...
	async function updateField(args: MutationTypeUserTest1ConnectionArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('UserTest1Connection', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
...
<SectionHead title="UserTest1Connection">
								...
								<div class="tooltip" data-tip={$LL.components.graphql.table.editBtn()}>
									<button
										...
										on:click={(e) => {
											e.preventDefault();
											if (node) {
												goto(`./user-test1-connection/${node.id}`);
											}
										}}
									>
