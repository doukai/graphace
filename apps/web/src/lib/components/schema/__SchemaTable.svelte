<script lang="ts">
	...
	import {
		...
		__Schema,
		__SchemaOrderBy,
		QueryType__SchemaListArgs,
		MutationType__SchemaArgs
	} from '~/gql/generated/schema';

	export let nodes: (__Schema | null | undefined)[] | null | undefined;
	export let totalCount: number = 0;
	export let isFetching: boolean;

	const dispatch = createEventDispatcher<{
		fetch: {
			args: QueryType__SchemaListArgs;
			then: (data: (__Schema | null | undefined)[] | null | undefined) => void;
			catch: (error: Error) => void;
		};
		mutation: {
			args: MutationType__SchemaArgs;
			then: (data: __Schema | null | undefined) => void;
			catch: (error: Error) => void;
		};
	}>();
	...
	let args: QueryType__SchemaListArgs = {};
	let orderBy: __SchemaOrderBy = {};
	...
	async function updateField(args: MutationType__SchemaArgs | null | undefined) {
		if (args && args.id) {
			errors[args.id] = {};
			validateUpdate('__Schema', args, $locale)
				.then((data) => {
					if (args) {
						dispatch('mutation', {
							args,
