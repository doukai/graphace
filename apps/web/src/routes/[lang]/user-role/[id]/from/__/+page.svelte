<script lang="ts">
	...
	import { Query_userConnectionStore, Mutation_userStore, Mutation_userRole_fromStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.UserRole.fields.from.name(), PageType.SELECT);
	...
	const select = (
		...
		validate('UserRole', { from: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_userRole_from.mutate({
						userRole_id: id,
						userRole_from: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
