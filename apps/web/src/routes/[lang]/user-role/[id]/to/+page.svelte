<script lang="ts">
	...
	import { Query_userRole_toStore, Mutation_userRole_toStore, Mutation_roleStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.UserRole.fields.to.name());
	...
	const parentMutation = (
		...
		validate('UserRole', { to: event.detail.args }, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_userRole_to.mutate({
					...
					.then((result) => {
						event.detail.then(result?.data?.userRole?.to);
					})
					.catch((errors) => {
						event.detail.catch(errors);
					});
