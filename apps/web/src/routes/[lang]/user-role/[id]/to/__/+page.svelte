<script lang="ts">
	...
	import { Query_roleConnectionStore, Mutation_roleStore, Mutation_userRole_toStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.UserRole.fields.to.name(), PageType.SELECT);
	...
	const select = (
		...
		validate('UserRole', { to: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_userRole_to.mutate({
						userRole_id: id,
						userRole_to: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
