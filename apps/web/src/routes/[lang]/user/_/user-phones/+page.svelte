<script lang="ts">
	...
	import UserPhonesCreateTable from '~/lib/components/objects/user-phones/UserPhonesCreateTable.svelte';
	...
	$: nodes = data.nodes as (MutationTypeUserPhonesArgs | null | undefined)[];
	...
	const mutation = (event: CustomEvent<{ nodes: (MutationTypeUserPhonesArgs | null | undefined)[] | null | undefined}> ) => {
		nodes = [...event.detail.nodes || []];
	};

	const save = (event: CustomEvent<{ nodes: (MutationTypeUserPhonesArgs | null | undefined)[] | null | undefined}> ) => {
		ot({
