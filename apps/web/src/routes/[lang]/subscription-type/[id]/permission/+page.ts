...
import { load_Query_subscriptionType_permission } from '$houdini';
...
export const load: LayoutLoad = async (event: LoadEvent) => {
    ...
    return {
        node,
        errors,
        ...(await load_Query_subscriptionType_permission({ event, variables: { subscriptionType_: { val: event.params.id } } }))
    };
}