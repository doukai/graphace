<script lang="ts">
	...
	import { Mutation_subscriptionType_userRoleListStore } from '$houdini';
	...
	const Mutation_subscriptionType_userRoleList = new Mutation_subscriptionType_userRoleListStore();
	...
	const mutation = (
		...
		validate('UserRole', event.detail.args, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_subscriptionType_userRoleList.mutate({
					subscriptionType_id: ,
					subscriptionType_userRoleList: [event.detail.args],
					update: true,
					mergeToList: ['userRoleList']
				})
					.then((result) => {
