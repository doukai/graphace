<script lang="ts">
	...
	import { Mutation_subscriptionType_roleListStore } from '$houdini';
	...
	const Mutation_subscriptionType_roleList = new Mutation_subscriptionType_roleListStore();
	...
	const mutation = (
		...
		validate('Role', event.detail.args, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_subscriptionType_roleList.mutate({
					subscriptionType_id: ,
					subscriptionType_roleList: [event.detail.args],
					update: true,
					mergeToList: ['roleList']
				})
					.then((result) => {
