...
import { load_Query_subscriptionType_userRole } from '$houdini';
...
export const load: LayoutLoad = async (event: LoadEvent) => {
    ...
    return {
        node,
        errors,
        ...(await load_Query_subscriptionType_userRole({ event, variables: { subscriptionType_: { val: event.params.id } } }))
    };
}