<script lang="ts">
	...
	import { Mutation_subscriptionType_userListStore } from '$houdini';
	...
	const Mutation_subscriptionType_userList = new Mutation_subscriptionType_userListStore();
	...
	const mutation = (
		...
		validate('User', event.detail.args, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_subscriptionType_userList.mutate({
					subscriptionType_id: ,
					subscriptionType_userList: [event.detail.args],
					update: true,
					mergeToList: ['userList']
				})
					.then((result) => {
