<script lang="ts">
	...
	import { Mutation_subscriptionType_userGroupListStore } from '$houdini';
	...
	const Mutation_subscriptionType_userGroupList = new Mutation_subscriptionType_userGroupListStore();
	...
	const mutation = (
		...
		validate('UserGroup', event.detail.args, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_subscriptionType_userGroupList.mutate({
					subscriptionType_id: ,
					subscriptionType_userGroupList: [event.detail.args],
					update: true,
					mergeToList: ['userGroupList']
				})
					.then((result) => {
