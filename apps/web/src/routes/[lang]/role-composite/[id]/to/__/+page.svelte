<script lang="ts">
	...
	import { Query_roleConnectionStore, Mutation_roleStore, Mutation_roleComposite_toStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.RoleComposite.fields.to.name(), PageType.SELECT);
	...
	const select = (
		...
		validate('RoleComposite', { to: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_roleComposite_to.mutate({
						roleComposite_id: id,
						roleComposite_to: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
