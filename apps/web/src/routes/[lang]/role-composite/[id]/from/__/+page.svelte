<script lang="ts">
	...
	import { Query_roleConnectionStore, Mutation_roleStore, Mutation_roleComposite_fromStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.RoleComposite.fields.from.name(), PageType.SELECT);
	...
	const select = (
		...
		validate('RoleComposite', { from: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_roleComposite_from.mutate({
						roleComposite_id: id,
						roleComposite_from: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
