<script lang="ts">
	...
	import RoleMenuCreateTable from '~/lib/components/objects/role-menu/RoleMenuCreateTable.svelte';
	...
	$: nodes = data.nodes as (MutationTypeRoleMenuArgs | null | undefined)[];
	...
	const mutation = (event: CustomEvent<{ nodes: (MutationTypeRoleMenuArgs | null | undefined)[] | null | undefined}> ) => {
		nodes = [...event.detail.nodes || []];
	};

	const save = (event: CustomEvent<{ nodes: (MutationTypeRoleMenuArgs | null | undefined)[] | null | undefined}> ) => {
		ot({
