<script lang="ts">
	...
	$: urlName($page.url, $LL.graphql.objects.User.fields.userRole.name(), PageType.SELECT);
	...
	const select = (
		...
		validateMutation('User', { id: id, userRole: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (Array.isArray(event.detail.selected)) {
					Mutation_user_userRole.mutate({
						user_id: id,
						user_userRole: event.detail.selected,
						update: true,
