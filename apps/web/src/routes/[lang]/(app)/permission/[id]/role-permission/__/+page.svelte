<script lang="ts">
	...
	import { Query_rolePermissionConnectionStore, Mutation_rolePermissionStore, Mutation_permission_rolePermissionStore } from '$houdini';
	...
	const select = (
		...
		validateMutation('Permission', { name: name, rolePermission: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (Array.isArray(event.detail.selected)) {
					Mutation_permission_rolePermission.mutate({
						permission_name: name,
						permission_rolePermission: event.detail.selected,
						update: true,
