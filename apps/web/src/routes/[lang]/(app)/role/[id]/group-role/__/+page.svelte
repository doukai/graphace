<script lang="ts">
	...
	import { Query_groupRoleConnectionStore, Mutation_groupRoleStore, Mutation_role_groupRoleStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.Role.fields.groupRole.name(), PageType.SELECT);
	...
	const select = (
		...
		validateMutation('Role', { id: id, groupRole: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (Array.isArray(event.detail.selected)) {
					Mutation_role_groupRole.mutate({
						role_id: id,
						role_groupRole: event.detail.selected,
						update: true,
