<script lang="ts">
	...
	import { Query_api_realmStore, Mutation_api_realmStore, Mutation_realmStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.Api.fields.realm.name());
	$: Query_api_realm = data.Query_api_realm as Query_api_realmStore;
	$: api = $Query_api_realm.data?.api;
	$: node = api?.realm;
	...
	const parentMutation = (
		...
		validateMutation('Api', { realm: event.detail.args }, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_api_realm.mutate({
					api_id: api?.id,
					api_realm: event.detail.args,
					update: true
				})
					.then((result) => {
						event.detail.then(result?.data?.api?.realm);
						if (result.errors) {
							event.detail.catch(result.errors);
						}
					});
...
{#if node}
	<RealmForm
		...
		isFetching={$Query_api_realm.fetching}
