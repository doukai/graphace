<script lang="ts">
	...
	import { Query_menu_realmStore, Mutation_menu_realmStore, Mutation_realmStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.Menu.fields.realm.name());
	$: Query_menu_realm = data.Query_menu_realm as Query_menu_realmStore;
	$: menu = $Query_menu_realm.data?.menu;
	$: node = menu?.realm;
	$: createNode = data.node;
	$: errors = data.errors as Record<string, Errors>;
	const Mutation_menu_realm = new Mutation_menu_realmStore();
	...
	const parentMutation = (
		...
		validateMutation('Menu', { realm: event.detail.args }, true, $locale)
			.then((data) => {
				errors = {};
				Mutation_menu_realm.mutate({
					menu_id: menu?.id,
					menu_realm: event.detail.args,
					update: true
				})
					.then((result) => {
						event.detail.then(result?.data?.menu?.realm);
						if (result.errors) {
							event.detail.catch(result.errors);
						}
					});
...
{#if node}
	<RealmForm
		...
		isFetching={$Query_menu_realm.fetching}
