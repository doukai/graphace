<script lang="ts">
	...
	import { Query_realmConnectionStore, Mutation_realmStore, Mutation_menu_realmStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.Menu.fields.realm.name(), PageType.SELECT);
	...
	const select = (
		...
		validateMutation('Menu', { realm: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_menu_realm.mutate({
						menu_id: id,
						menu_realm: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
