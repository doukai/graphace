<script lang="ts">
	...
	import { Query_roleConnectionStore, Mutation_roleStore, Mutation_menu_roleStore } from '$houdini';
	...
	$: urlName($page.url, $LL.graphql.objects.Menu.fields.role.name(), PageType.SELECT);
	...
	const select = (
		...
		validateMutation('Menu', { role: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_menu_role.mutate({
						menu_id: id,
						menu_role: event.detail.selected,
						update: true
					})
						.then((result) => {
							event.detail.then();
