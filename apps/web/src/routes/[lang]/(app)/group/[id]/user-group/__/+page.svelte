<script lang="ts">
	...
	$: urlName($page.url, $LL.graphql.objects.Group.fields.userGroup.name(), PageType.SELECT);
	...
	const mutation = (
		...
		validateMutation('UserGroup', event.detail.args, event.detail.update, $locale)
			.then((data) => {
	...
	const select = (
		...
		validateMutation('Group', { id: id, userGroup: event.detail.selected }, true, $locale)
			.then((data) => {
				errors = {};
