<script lang="ts">
	...
	const query = (to?: number | undefined, params?: FetchParams | undefined) => {
		...
		query_role_groupsConnection_Store
...
<Breadcrumbs>
	...
	<li>
		<span class="badge badge-neutral">{$LL.graphql.objects.Role.fields.groups.name()}</span>
	</li>
</Breadcrumbs>
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<GroupTable
			...
			on:create={(e) => to(`/${$locale}/role/${role?.id}/groups/_`)}
