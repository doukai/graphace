<script lang="ts">
	...
	$: query_role_compositesConnection_Store = data.query_role_compositesConnection_Store;
	...
	const query = (to?: number | undefined, params?: FetchParams | undefined) => {
		...
		query_role_compositesConnection_Store
	...
	const mutation_composites = (input: RoleInput[], params?: FetchParams | undefined) => {
		validate('Mutation_role_Arguments', { where: { id: { val: role?.id } }, composites: input })
			.then((data) => {
				errors = {};
...
<Breadcrumbs>
	...
	<li>
		<span class="badge badge-neutral">{$LL.graphql.objects.Role.fields.composites.name()}</span>
	</li>
</Breadcrumbs>
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<RoleTable
			...
			on:create={(e) => to(`/${$locale}/role/${role?.id}/composites/_`)}
			on:goto={(e) => to(`/${$locale}/role/${e.detail.path}`)}
			on:back={(e) => ot()}
		>
			{#if auth('Role::*::WRITE')}
				<RoleTableDialog
					args={{ exs: [{ not: true, composites: { id: { val: role?.id } } }] }}
					class="btn-accent"
					on:select={(e) => {
						if (Array.isArray(e.detail.value)) {
							mutation_composites(e.detail.value.filter((item) => item != null));
						}
					}}
				>
