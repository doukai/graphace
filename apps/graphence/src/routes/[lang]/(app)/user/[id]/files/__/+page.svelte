<script lang="ts">
	...
	import { Query_fileConnectionStore, Mutation_fileStore, Mutation_user_filesStore } from '$houdini';
	...
	const select = (
		...
		validate('Mutation_user_Arguments', { where: { id: { val: id } }, files: event.detail.selected }, $locale)
			.then((data) => {
				errors = {};
				if (Array.isArray(event.detail.selected)) {
					Mutation_user_files.mutate({
						user_id: id,
						user_files: event.detail.selected
					})
						.then((result) => {
							if (result.errors) {
								event.detail.catch(result.errors);
