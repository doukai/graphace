<script lang="ts">
	...
	$: query_user_rolesConnection_Store = data.query_user_rolesConnection_Store;
	...
	$: mutation_user_roles_Store = data.mutation_user_roles_Store;
	...
	const query = (to?: number | undefined, params?: FetchParams | undefined) => {
		...
		query_user_rolesConnection_Store
	...
	const mutation_roles = (input: RoleInput[], params?: FetchParams | undefined) => {
		validate('Mutation_user_Arguments', { where: { id: { val: user?.id } }, roles: input })
			.then((data) => {
				errors = {};
				mutation_user_roles_Store
					.fetch(
						{
							user_id: id,
							user_roles: input
...
<Breadcrumbs>
	...
	<li>
		<span class="badge badge-neutral">{$LL.graphql.objects.User.fields.roles.name()}</span>
	</li>
</Breadcrumbs>
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<RoleTable
			...
			isMutating={$validator.isValidating || $mutation_user_roles_Store.isFetching || $mutation_role_Store.isFetching}
			on:search={(e) => {
				if (e.detail.value) {
					args = {
						cond: 'OR',
			...
			on:create={(e) => to(`/${$locale}/user/${user?.id}/roles/_`)}
