<script lang="ts">
	...
	const mutation_realm = (input: RealmInput | null) => {
		validate('Mutation_user_Arguments', { where: { id: { val: user?.id } }, realm: input })
			.then((data) => {
				errors = {};
				mutation_user_realm_Store
					.fetch(
						{
							user_id: id,
							user_realm: input
...
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<RealmForm
			...
			isMutating={$validator.isValidating || $mutation_user_realm_Store.isFetching || $mutation_realm_Store.isFetching}
			on:save={(e) => {
				if (e.detail.value) {
