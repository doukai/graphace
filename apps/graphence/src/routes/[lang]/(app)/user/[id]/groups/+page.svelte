<script lang="ts">
	...
	const query = (to?: number | undefined) => {
		...
		query_user_groupsConnection_Store.fetch({ user_id: user?.id, ...buildArguments(args) }).then((result) => {
			if (result.errors) {
	...
	const merge = (args: GroupInput[]) => {
		validating = true;
		validate('Mutation_user_Arguments', { where: { id: { val: user?.id } }, groups: args }, $locale)
			.then((data) => {
				validating = false;
				errors = {};
...
<Card>
	<CardBody>
		<GroupTable
			...
			isMutating={validating || $mutation_user_groups_Store.isFetching || $mutation_group_Store.isFetching}
			fields={{
				name: {
