<script lang="ts">
	...
	const mutation_roles = (input: RoleInput, params?: FetchParams | undefined) => {
		validate('Mutation_user_Arguments', { where: { id: { val: id } }, roles: [input] })
			.then((data) => {
				errors = {};
...
<Breadcrumbs>
	...
	<li>
		<a href={undefined} on:click|preventDefault={(e) => to(`/${$locale}/user/${id}/roles`)}>
			<span class="badge badge-outline">{$LL.graphql.objects.User.fields.roles.name()}</span>
		</a>
	</li>
	<li>
...
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<RoleForm
			...
			{value}
			{errors}
			isMutating={$validator.isValidating || $mutation_user_roles_Store.isFetching}
			on:save={(e) => {
				if (e.detail.value) {
					mutation_roles(e.detail.value);
				}
			}}
			on:back={(e) => ot()}
		/>
