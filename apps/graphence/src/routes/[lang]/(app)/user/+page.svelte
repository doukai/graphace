<script lang="ts">
	...
	import { createMutation_userList_Store } from '~/lib/stores/mutation/mutation_userList_store';
	import {
		validator,
		permissions,
	...
	const export_userConnection_Store = createQuery_userConnection_Store($loadEvent);
	const import_userList_Store = createMutation_userList_Store($loadEvent);
	...
	const query = (to?: number | undefined, params?: FetchParams | undefined) => {
		...
		query_userConnection_Store.fetch(buildArguments(args), params).then((result) => {
			if (result.errors) {
	...
	<ModuleMenu>
				...
				export_userConnection_Store
					...
					.then((result) => {
						if (result.errors) {
							...
							const nodes = result.data?.userConnection?.edges?.map((edge) => edge?.node);
							const json = toRecords($LL, nodes);
							if (json) {
								e.detail.writeFile($LL.graphql.objects.User.name(), json);
							}
						}
					});
			}}
		...
		<ImportDialog
			on:import={(e) => {
				...
				if (list) {
					validate('Mutation_userList_Arguments', { list })
						.then((data) => {
							import_userList_Store
