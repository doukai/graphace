<script lang="ts">
	...
	import { SideBarMenu, UserMenu } from '~/lib/components/menu';
	...
	setContext('ui.card', 'bg-base-100');
	setContext('ui.card-body', 'max-sm:p-2');
	setContext('ui.dropdown-content', 'bg-base-200 rounded-box shadow');
	setContext('ui.popover-content', 'bg-base-200 rounded-box shadow');
	setContext('ui.tab', '[&_[data-part=list]]:tab-bordered');
	setContext('ui.drawer', 'gap-1');
	setContext('ui.breadcrumbs', 'overflow-y-hidden min-h-8');
...
<Drawer bind:open let:id class="lg:drawer-open h-screen overflow-hidden">
	<DrawerContent {id} class="flex flex-col flex-1 min-w-0 min-h-0 overflow-hidden bg-base-200">
			...
			<NavBarStart class="flex items-center space-x-1">
				...
				>
					...
					<div class="inline-flex text-lg md:text-2xl">
						<span class="capitalize text-neutral">Graph</span>
		...
		<main class="flex flex-col flex-1 min-h-0 min-w-0 p-1 md:p-2">
			<slot />
			<Toasts />
			<Modals />
			{#if $navigating}
				<LoadingModal message={$LL.graphence.message.loading()} isOpen zIndex={$zIndex + 3} />
			{/if}
		</main>
	</DrawerContent>
