<script lang="ts">
	...
	const query = (args: QueryUserConnectionArgs) => {
		query_group_usersConnection_Store.fetch({ group_id: group?.id, ...args }).then((result) => {
			if (result.errors) {
		...
		<UserTable
			...
			on:goto={(e) => {
				if (Array.isArray(e.detail.path)) {
					add(`./users/${e.detail.path[0].path}`, e.detail.path[0].name);
					to(`./users/${e.detail.path[1].path}`, e.detail.path[1].name);
				}
			}}
			on:back={(e) => ot()}
		/>
