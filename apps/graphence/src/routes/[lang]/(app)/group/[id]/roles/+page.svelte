<script lang="ts">
	...
	$: query_group_rolesConnection_Store = data.query_group_rolesConnection_Store;
	...
	const query = (to?: number | undefined, params?: FetchParams | undefined) => {
		...
		query_group_rolesConnection_Store
...
<Breadcrumbs>
	...
	<li>
		<span class="badge badge-neutral">{$LL.graphql.objects.Group.fields.roles.name()}</span>
	</li>
</Breadcrumbs>
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<RoleTable
			...
			on:create={(e) => to(`/${$locale}/group/${group?.id}/roles/_`)}
