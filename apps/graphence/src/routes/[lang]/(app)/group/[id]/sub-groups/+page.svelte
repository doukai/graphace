<script lang="ts">
	...
	const query = (args: QueryGroupConnectionArgs) => {
		query_group_subGroupsConnection_Store.fetch({ group_id: group?.id, ...args }).then((result) => {
			if (result.errors) {
...
<Card>
	<CardBody>
		<GroupTable
			...
			on:create={(e) => to('./sub-groups/_')}
			on:goto={(e) => {
				if (Array.isArray(e.detail.path)) {
					add(`./sub-groups/${e.detail.path[0].path}`, e.detail.path[0].name);
					to(`./sub-groups/${e.detail.path[1].path}`, e.detail.path[1].name);
				}
			}}
			on:back={(e) => ot()}
		/>
