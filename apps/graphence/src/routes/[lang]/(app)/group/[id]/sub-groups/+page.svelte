...
<Card class="flex flex-col max-w-full min-h-0">
	<CardBody class="flex-1 min-h-0">
		<GroupTable
			...
			isMutating={$validator.isValidating || $mutation_group_subGroups_Store.isFetching || $mutation_group_Store.isFetching}
			on:search={(e) => {
				if (e.detail.value) {
					args = {
						cond: 'OR',
			...
			on:create={(e) => to(`/${$locale}/group/${group?.id}/subGroups/_`)}
		...
		>
			{#if auth('Group::*::WRITE')}
				<GroupTableDialog
					...
					on:select={(e) => {
						if (Array.isArray(e.detail.value)) {
							mutation_subGroups(e.detail.value.filter((item) => item != null));
						}
					}}
				>
