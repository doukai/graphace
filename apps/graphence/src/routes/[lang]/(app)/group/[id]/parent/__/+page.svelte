<script lang="ts">
	...
	import { Query_groupConnectionStore, Mutation_groupStore, Mutation_group_parentStore } from '$houdini';
	...
	const select = (
		...
		validate('Mutation_group_Arguments', { where: { id: { val: id } }, parent: event.detail.selected }, $locale)
			.then((data) => {
				errors = {};
				if (!Array.isArray(event.detail.selected)) {
					Mutation_group_parent.mutate({
						group_id: id,
						group_parent: event.detail.selected
					})
						.then((result) => {
							if (result.errors) {
								event.detail.catch(result.errors);
