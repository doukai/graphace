...
import type { File, FileConnection } from '~/lib/types/schema';
...
export const load: LayoutLoad = async (event: LoadEvent) => {
    ...
    return {
        ...
        showOptionButton,
        showFilterButton,
        query_fileConnection_Store: (await fetchQueryStore<FileConnection>(event, { fields: [createConnectionField({ name: 'fileConnection', fields })] })),
        mutation_fileList_Store: (createMutationStore<File[]>(event))
    };
}
