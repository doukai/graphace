<script lang="ts">
	...
	$: if (
		...
		queryNamed_realmList_Store.fetch({ id: { opr: 'IN', arr: value?.arr } }).then(
			(response) =>
				(selectedList = response.data?.realmList?.map((item) => ({
					label: item?.name,
					value: item?.id,
					node: item
				})))
		...
		queryNamed_realmList_Store.fetch({ id: { opr: 'EQ', val: value?.val } }).then(
			(response) =>
		...
		<ObjectSelect
			...
			on:search={(e) => {
				if (e.detail.searchValue) {
					queryNamed_realmList_Store.fetch({ name: { opr: 'LK', val: `%${e.detail.searchValue}%` } });
				} else {
					queryNamed_realmList_Store.fetch({ name: undefined, first: 10 });
				}
			}}
		/>
	{:else}
