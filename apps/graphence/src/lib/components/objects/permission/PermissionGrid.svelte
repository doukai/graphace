<script lang="ts">
	...
	$: if (nodes) {
		...
		source = nodes?.flatMap((node) => {
			if (join) {
				const array = node?.[join.name as keyof Permission];
					...
					return array
						.map((item) => {
							if (join.fields && join.fields.length > 0) {
								...
								return [[join.name, node?.[join.name as keyof Permission]]];
							}
						})
						.map((item) =>
							Object.fromEntries([
								...
								...queryFields
									...
									.flatMap((field) => {
										if (field.fields && field.fields.length > 0) {
											...
											return [[field.name, node?.[field.name as keyof Permission]]];
										}
									})
							])
						);
		...
		if (join) {
			list = source?.reduce((nodes: DataType[], row) => {
				...
				if (
					...
					row['name'] &&
					nodes.some((node: DataType) => node['name'] === row['name'])
				) {
					nodes
						.find((node: DataType) => node['name'] === row['name'])
						?.[join.name].push(
							Object.fromEntries(
								join!.fields?.map((subField) => [
									subField.name,
