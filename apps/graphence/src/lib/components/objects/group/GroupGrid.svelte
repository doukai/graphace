<script lang="ts">
	...
	import type { Group, QueryGroupListArgs, MutationGroupListArgs } from '~/lib/types/schema';
	import {
		__schema,
		getGridTheme,
		createEditors,
	...
	export let buildArguments: () => QueryGroupListArgs;
	...
	const dispatch = createEventDispatcher<{
		...
		exportQuery: {
			...
			directives?: Directive[];
			then?: (value: (Group | null | undefined)[] | null | undefined) => void;
		};
	}>();
	...
	$: source = nodesToSource<Group>(typeName, queryFields, value) || [];
	$: gridErrors = errorsToGridErrors<Group>(typeName, errors, queryFields, value);
