<script lang="ts">
	...
	$: if (
		...
		queryNamed_groupList_Store.fetch({ id: { opr: 'IN', arr: value?.arr } }).then(
			(response) =>
		...
		queryNamed_groupList_Store.fetch({ id: { opr: 'EQ', val: value?.val } }).then(
			(response) =>
		...
		<ObjectSelect
			...
			on:search={(e) => {
				if (e.detail.searchValue) {
					queryNamed_groupList_Store.fetch({ name: { opr: 'LK', val: `%${e.detail.searchValue}%` } });
				} else {
					queryNamed_groupList_Store.fetch({ name: undefined, first: 10 });
				}
			}}
		/>
	{:else}
