<script lang="ts">
	...
	import type { Role, QueryRoleListArgs, MutationRoleListArgs } from '~/lib/types/schema';
	import {
		__schema,
		getGridTheme,
		createEditors,
	...
	export let buildArguments: () => QueryRoleListArgs;
	...
	const dispatch = createEventDispatcher<{
		...
		exportQuery: {
			...
			directives?: Directive[];
			then?: (value: (Role | null | undefined)[] | null | undefined) => void;
		};
	}>();
	...
	$: source = nodesToSource<Role>(typeName, queryFields, value) || [];
	$: gridErrors = errorsToGridErrors<Role>(typeName, errors, queryFields, value);
