<script lang="ts">
	...
	import type { UserOrderBy, QueryUserConnectionArgs, UserInput } from '~/lib/types/schema';
						...
						<tr class="hover">
							...
							{#if permissions.auth('User::name::*')}
								<StringTd
									...
									on:save={(e) =>
										updateField({ name: node?.name, where: { id: { val: node?.id } } }, row)}
							...
							{#if permissions.auth('User::description::*')}
								<StringTd
									...
									on:save={(e) =>
										updateField(
											{ description: node?.description, where: { id: { val: node?.id } } },
											row
										)}
							...
							{#if permissions.auth('User::lastName::*')}
								<StringTd
									...
									on:save={(e) =>
										updateField(
											{ lastName: node?.lastName, where: { id: { val: node?.id } } },
											row
										)}
							...
							{#if permissions.auth('User::login::*')}
								<StringTd
									...
									on:save={(e) =>
										updateField({ login: node?.login, where: { id: { val: node?.id } } }, row)}
							...
							{#if permissions.auth('User::email::*')}
								<StringTd
									...
									on:save={(e) =>
										updateField({ email: node?.email, where: { id: { val: node?.id } } }, row)}
							...
							{#if permissions.auth('User::phones::*')}
								<StringTd
									...
									list
									on:save={(e) =>
										updateField({ phones: node?.phones, where: { id: { val: node?.id } } }, row)}
							...
							{#if permissions.auth('User::disable::*')}
								<BooleanTd
									...
									on:save={(e) =>
										updateField({ disable: node?.disable, where: { id: { val: node?.id } } }, row)}
