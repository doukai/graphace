<script lang="ts">
	...
	import type { User, QueryUserListArgs, MutationUserListArgs } from '~/lib/types/schema';
	import {
		__schema,
		getGridTheme,
		createEditors,
	...
	export let buildArguments: () => QueryUserListArgs;
	...
	const dispatch = createEventDispatcher<{
		...
		exportQuery: {
			...
			directives?: Directive[];
			then?: (value: (User | null | undefined)[] | null | undefined) => void;
		};
	}>();
	...
	$: source = nodesToSource<User>(typeName, queryFields, value) || [];
	$: gridErrors = errorsToGridErrors<User>(typeName, errors, queryFields, value);
