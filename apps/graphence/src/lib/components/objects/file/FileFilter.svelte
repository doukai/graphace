<script lang="ts">
	...
	import type { FileExpression } from '~/lib/types/schema';
	...
	<div class="z-[{zIndex + 5}] {className} {contextClass}" use:melt={$content}>
		...
		<Form class="max-h-60 overflow-y-auto">
			<FormControl let:id>
				...
				<div class="grid grid-cols-2 gap-1">
					<StringFilter {id} name="contentType" bind:value={value.contentType} />
				</div>
				<Label {id} text={$LL.graphql.objects.File.fields.content.name()} />
				<div class="grid grid-cols-2 gap-1">
					<StringFilter {id} name="content" bind:value={value.content} />
				</div>
				<Label {id} text={$LL.graphql.objects.File.fields.url.name()} />
				<div class="grid grid-cols-2 gap-1">
					<StringFilter {id} name="url" bind:value={value.url} />
				</div>
			</FormControl>
		</Form>
