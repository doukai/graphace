<script lang="ts">
	...
	export let value: FileInput | (FileInput | null | undefined)[] | null | undefined =
		undefined;
	...
	export let tabs: (($LL: TranslationFunctions, args?: QueryFileListArgs | undefined) => TabInfo[] | undefined) | undefined = undefined;
	...
	const query_fileConnection_Store = createQuery_fileConnection_Store($loadEvent);
	...
	let _value: (FileInput | null | undefined)[] = [];
	$: if (nodes && nodes.length > 0) {
		_value = nodes;
	} else {
		_value = [];
	}
	export let args: QueryFileConnectionArgs = {};
		...
		if (Array.isArray(value)) {
			if (value.some((item) => !item?.[textFieldName] && item?.id)) {
				query_fileConnection_Store
					...
					.then((response) => {
						value = response.data?.fileConnection?.edges?.map((edge) => ({
...
<Dialog bind:close>
	...
	<svelte:fragment let:zIndex>
		<FileTable
			...
			showCreateButton={!readonly && auth('File::*::WRITE')}
