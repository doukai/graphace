<script lang="ts">
	...
	import type { File, QueryFileListArgs, MutationFileListArgs } from '~/lib/types/schema';
	import {
		__schema,
		getGridTheme,
		createEditors,
	...
	export let buildArguments: () => QueryFileListArgs;
	...
	const dispatch = createEventDispatcher<{
		...
		exportQuery: {
			...
			directives?: Directive[];
			then?: (value: (File | null | undefined)[] | null | undefined) => void;
		};
	}>();
	...
	$: source = nodesToSource<File>(typeName, queryFields, value) || [];
	$: gridErrors = errorsToGridErrors<File>(typeName, errors, queryFields, value);
