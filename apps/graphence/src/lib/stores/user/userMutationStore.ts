...
export async function createUserAggStore(params: { event: LoadEvent, fields: Field[], mutationArguments: UserListMutationArguments }): Promise<UserMutationStore> {
    ...
    const fetch = async (fields: Field[], mutationArguments: UserListMutationArguments) => {
        if (fields && fields.length > 0 && mutationArguments.list && mutationArguments.list.length > 0) {
