...
import type { QueryRealmListArgs, Role } from '~/lib/types/schema';
...
export async function createQuery_role_realm_Store(event: LoadEvent, variables: { role_id: string } & QueryRealmListArgs): Promise<Query_role_realm_Store> {
  return createGraphQLQueryStore<Role, { role_id: string } & QueryRealmListArgs>(query, event, variables);
}

export type Query_role_realm_Store = GraphQLStore<Role, { role_id: string } & QueryRealmListArgs>;