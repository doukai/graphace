...
export function createQuery_permission_Store(event: Event): Query_permission_Store {
  return createGraphQLQueryStore<{ permission: Permission }, QueryPermissionArgs>(query, event);
}
...
export async function fetchQuery_permission_Store(event: Event, variables: QueryPermissionArgs): Promise<Query_permission_Store> {
  return fetchGraphQLQueryStore<{ permission: Permission }, QueryPermissionArgs>(query, event, variables);
}

export type Query_permission_Store = GraphQLStore<{ permission: Permission }, QueryPermissionArgs>;