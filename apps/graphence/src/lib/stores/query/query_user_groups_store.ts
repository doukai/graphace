...
import type { QueryUserArgs, Group } from '~/lib/types/schema';
...
export async function createQueryUserStore(event: LoadEvent, variables: { user_id: string } & QueryUserArgs): Promise<GraphQLStore<Group, { user_id: string } & QueryUserArgs>> {
  return createGraphQLQueryStore<Group, QueryUserArgs>(query, event, variables);
}