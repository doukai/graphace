...
import type { QueryUserArgs, Realm } from '~/lib/types/schema';
...
export async function createQueryUserStore(event: LoadEvent, variables: { user_id: string } & QueryUserArgs): Promise<GraphQLStore<Realm, { user_id: string } & QueryUserArgs>> {
  return createGraphQLQueryStore<Realm, QueryUserArgs>(query, event, variables);
}