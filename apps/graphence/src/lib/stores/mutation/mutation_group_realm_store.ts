...
import type { MutationRealmListArgs, Group } from '~/lib/types/schema';
...
export async function createMutation_group_realm_Store(event: LoadEvent): Promise<Mutation_group_realm_Store> {
  return createGraphQLMutationStore<Group, { group_id: string } & MutationRealmListArgs>(query, event);
}

export type Mutation_group_realm_Store = GraphQLStore<Group, { group_id: string } & MutationRealmListArgs>;