...
import type { MutationRealmArgs, Role } from '~/lib/types/schema';
...
export async function createMutation_role_realm_Store(event: LoadEvent): Promise<Mutation_role_realm_Store> {
  return createGraphQLMutationStore<Role, { role_id: string } & MutationRealmArgs>(query, event);
}

export type Mutation_role_realm_Store = GraphQLStore<Role, { role_id: string } & MutationRealmArgs>;