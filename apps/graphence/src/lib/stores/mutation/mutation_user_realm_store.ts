...
import type { MutationRealmArgs, User } from '~/lib/types/schema';
...
export async function createMutation_user_realm_Store(event: LoadEvent): Promise<Mutation_user_realm_Store> {
  return createGraphQLMutationStore<User, { user_id: string } & MutationRealmArgs>(query, event);
}

export type Mutation_user_realm_Store = GraphQLStore<User, { user_id: string } & MutationRealmArgs>;