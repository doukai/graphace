...
import type { MutationUserArgs, Realm } from '~/lib/types/schema';
...
export async function createMutationUserStore(event: LoadEvent): Promise<GraphQLStore<Realm, { user_id: string } & MutationUserArgs>> {
  return createGraphQLMutationStore<Realm, MutationUserArgs>(query, event);
}