...
const query = ({ fields, directives }: QueryParams) =>/* GraphQL */ `mutation Mutation_role_groups($role_id: String, $role_groups: [GroupInput]) {
  role(where: { id: { val: $role_id } }, groups: $role_groups) ${directives}{
    ...RoleFields
    syncRolePolicy
    groups {
      ...GroupFields
