...
import type { MutationRoleArgs, Group } from '~/lib/types/schema';
...
export async function createMutationRoleStore(event: LoadEvent): Promise<GraphQLStore<Group, { role_id: string } & MutationRoleArgs>> {
  return createGraphQLMutationStore<Group, MutationRoleArgs>(query, event);
}