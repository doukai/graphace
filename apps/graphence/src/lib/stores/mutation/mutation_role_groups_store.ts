...
export async function createMutation_role_groups_Store(event: LoadEvent): Promise<Mutation_role_groups_Store> {
  return createGraphQLMutationStore<Role, { role_id: string } & MutationGroupConnectionArgs>(query, event);
}

export type Mutation_role_groups_Store = GraphQLStore<Role, { role_id: string } & MutationGroupConnectionArgs>;