...
import type { Role, RoleListMutationArguments } from '~/';
...
export async function createRoleListMutationStore(params: { event: LoadEvent }): Promise<RoleListMutationStore> {
    ...
    const fetch = async (fields: Field[], mutationArguments: RoleListMutationArguments, directives?: Directive[]) => {
...
export type RoleListMutationStore = {
    ...
    fetch: (fields: Field[], mutationArguments: RoleListMutationArguments, directives?: Directive[]) => Promise<{ data?: { roleList: Role[] | null | undefined }, errors?: GraphQLError[] | null | undefined } | null | undefined>;
}